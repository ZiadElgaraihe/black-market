...
class GetIngotsDataBlocConsumer extends StatelessWidget {
  const GetIngotsDataBlocConsumer({super.key});
        ...
        if (state is GetIngotsCoinsDataLoading) {
          ...
          return RefreshIndicator(
            ...
            child: ListView.separated(
              ...
              itemCount: getIngotsCoinsDataCubit.ingots?.length ?? 12,
              itemBuilder: (context, index) => GoldListItemExpansionTile(
                weight: getIngotsCoinsDataCubit.ingots?[index].weight,
                company: getIngotsCoinsDataCubit
                    .ingots?[index].companies.firstOrNull,
                price: getIngotsCoinsDataCubit.ingots?[index].price,
              ),
              separatorBuilder: (context, index) => SizedBox(height: 18.h),
            ),
          );
